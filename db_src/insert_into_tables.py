...
def process_password(password):
    ...
    hashed = hashlib.md5(saltPw.encode()).hexdigest()
    return hashed
    ...
    with open(file_path, newline='', encoding="utf-8") as csvfile:
        ...
        for row in csv_reader:
            ...
            if (row[4]):
                ...
                review_img = None
            dic_tuples_reviews[index] = (user_id, location_id, stars, comment, review_img, upvote, downvote)
            index += 1
...
def insert_table(location_dir, review_dir, user_dir, config_file, location_table, review_table, user_table):
    ...
    for entry in dic_tuples_locations:
        cursor_object.execute(
            "INSERT INTO MUSE_DB." + location_table + " (location_name, location_type, address, description, location_image)"
                                                      "values (%s, %s, %s, %s, %s)", (dic_tuples_locations[entry]))

    for entry in dic_tuples_reviews:
        cursor_object.execute(
            "INSERT INTO MUSE_DB." + review_table + " (user_id, location_id, stars, comment, review_image, upvote, downvote)"
                                                    "values (%s, %s, %s, %s, %s, %s, %s)", (dic_tuples_reviews[entry]))
...
path_to_location_dir = "../will/csv/location/"
path_to_review_dir = "../will/csv/review/"
path_to_user_dir = "../will/csv/user/"
insert_table(path_to_location_dir, path_to_review_dir, path_to_user_dir, "../will/connectorConfig.json", "artLocation",
             "review", "user")
